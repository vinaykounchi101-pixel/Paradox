...
from app.core.exceptions import NotFoundError, ValidationError, UnprocessableRequestError
...
class ExpenseService:
    ...
    async def _validate_references(self, category_id: uuid.UUID, payment_method_id: uuid.UUID) -> None:
    ...
    async def get_expense(self, id: uuid.UUID) -> Expense:
        expense = await self.repo.get_by_id(id)
        if not expense:
    ...
    async def create_expense(self, data: ExpenseCreate) -> Expense:
        ...
        await self._validate_references(data.category_id, data.payment_method_id)
    ...
    async def update_expense(self, id: uuid.UUID, data: ExpenseUpdate) -> Expense:
        expense = await self.get_expense(id)
        ...
        if data.category_id is not None or data.payment_method_id is not None:
            await self._validate_references(category_id, payment_method_id)
    ...
    async def delete_expense(self, id: uuid.UUID) -> None:
        expense = await self.get_expense(id)
