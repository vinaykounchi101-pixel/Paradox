...
class DashboardService:
    ...
    async def get_dashboard_data(self, user_id: uuid.UUID, period: str = "current_month") -> DashboardRead:
        ...
        # 2. Fetch expenses within the selected period strictly for user_id
        expenses = await self.expense_repo.get_expenses_for_period(user_id, date_from, date_to)
        ...
        if period == "current_week":
            ...
            budget = await self.budget_repo.get_budget(user_id, "week", week_key)
            ...
            if period == "current_month":
                ...
                month_expenses = await self.expense_repo.get_expenses_for_period(user_id, month_start, today)
            spent_for_budget = sum((e.amount for e in month_expenses), Decimal("0.00"))
            budget = await self.budget_repo.get_budget(user_id, "month", today.strftime("%Y-%m"))
        ...
        # 6. Fetch 5 most recent expenses (scoped to user_id)
