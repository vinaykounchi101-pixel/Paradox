...
@pytest.mark.asyncio
async def test_email_service_send_brevo_dispatch():
    service = EmailService()

    with patch.object(settings, "BREVO_API_KEY", "xkeysib-test-12345"):
        with patch.object(settings, "BREVO_SENDER_EMAIL", "brevo_sender@example.com"):
            with patch.object(service, "_send_brevo_email_sync", return_value=True) as mock_brevo:
                result = await service.send_registration_verification_email("new_user@example.com", "token-brevo-999")
                assert result is True
                mock_brevo.assert_called_once()
                call_kwargs = mock_brevo.call_args.kwargs
                assert call_kwargs["to_email"] == "new_user@example.com"
                assert "token-brevo-999" in call_kwargs["html_body"]
...
async def test_email_service_provider_selection():
    ...
    # When Brevo is configured, auto prioritizes Brevo
    with patch.object(settings, "BREVO_API_KEY", "xkeysib-123"):
        with patch.object(settings, "RESEND_API_KEY", None):
            with patch.object(settings, "EMAIL_PROVIDER", "auto"):
                assert service.active_provider == "brevo"

    # When Resend is configured and Brevo is absent
    with patch.object(settings, "BREVO_API_KEY", None):
